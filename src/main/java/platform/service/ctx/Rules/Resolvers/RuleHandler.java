...
import platform.service.ctx.Resolvers.ResolverType;
...
public class RuleHandler {
    ...
    public HashMap<String, Resolver> buildRules(String filename) throws Exception {
        ...
        HashMap<String, Resolver> ruleId2Resolver = new HashMap<>();
        for(Element Erule: Erulelist){
            List<Element> Enodelist = Erule.elements();
            if(Enodelist.size() != 3){
                // id + formula + resolver
                logger.warning("[CCE] rule does not have 3 root node but " + Enodelist.size());
                continue;
            }
            ...
            // resolver
            ruleId2Resolver.put(newrule.getRule_id(), buildResolver(Enodelist.get(2).elements()));
        }
        return ruleId2Resolver;
    }

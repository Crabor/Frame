...
import platform.service.ctx.ctxChecker.middleware.schedulers.GEAS_opt_c;
...
public class FForall extends Formula{
    ...
    //DIS
    private final Map<ContextChange.ChangeType, Set<RuntimeNode.Virtual_Truth_Type>> rcSet;
    private final Map<ContextChange.ChangeType, Set<RuntimeNode.Virtual_Truth_Type>> reSet;
    ...
    //getter and setter
    public Map<ContextChange.ChangeType, Set<RuntimeNode.Virtual_Truth_Type>> getReSet() {
        return reSet;
    }

    public Map<ContextChange.ChangeType, Set<RuntimeNode.Virtual_Truth_Type>> getRcSet() {
        return rcSet;
    }
...
//                + "\tentireWorkload: " + this.getEntireWorkload().getKey() + "-" + this.getEntireWorkload().getValue()
    ...
    public void DeriveIncPlusSet(Set<Map.Entry<ContextChange.ChangeType, String>> incPlusSet) {
        incPlusSet.add(new AbstractMap.SimpleEntry<>(ContextChange.ChangeType.ADDITION, pattern_id));
    ...
    public void DeriveIncMinusSet(Set<Map.Entry<ContextChange.ChangeType, String>> incMinusSet) {
        incMinusSet.add(new AbstractMap.SimpleEntry<>(ContextChange.ChangeType.DELETION, pattern_id));
    ...
    public boolean EvaluationAndEqualSideEffect(RuntimeNode curNode, Formula originFormula, String var, ContextChange delChange, ContextChange addChange, boolean canConcurrent, Scheduler scheduler) {
        ...
        if(curNode.getChildren().size() == 0)
            return false;
        if(delChange.getPatternId().equals(this.pattern_id)){
            assert var == null;
            int meet_cnt = 0;
    ...
    public void DeriveRCRESets(boolean from) {
        if(from){
            //T to F
            this.rcSet.put(ContextChange.ChangeType.ADDITION, new HashSet<RuntimeNode.Virtual_Truth_Type>(){
                {
                    add(RuntimeNode.Virtual_Truth_Type.UNKNOWN);
                }
            });
            this.reSet.put(ContextChange.ChangeType.DELETION, new HashSet<RuntimeNode.Virtual_Truth_Type>(){
                {
                    add(RuntimeNode.Virtual_Truth_Type.UNKNOWN);
                }
            });
            ...
            //F to T
            this.rcSet.put(ContextChange.ChangeType.DELETION, new HashSet<RuntimeNode.Virtual_Truth_Type>(){
                {
                    add(RuntimeNode.Virtual_Truth_Type.UNKNOWN);
                    add(RuntimeNode.Virtual_Truth_Type.FALSE);
                }
            });
            this.reSet.put(ContextChange.ChangeType.ADDITION, new HashSet<RuntimeNode.Virtual_Truth_Type>(){
                {
                    add(RuntimeNode.Virtual_Truth_Type.UNKNOWN);
                }
            });
    ...
    public boolean UpdateAffectedWithOneChange(ContextChange contextChange, Checker checker) {
        if(contextChange.getPatternId().equals(this.pattern_id)){
...
//        }
    ...
    public void ModifyBranch_PCC(String rule_id, RuntimeNode curNode, Formula originFormula, ContextChange contextChange, Checker checker) {
        if(contextChange.getPatternId().equals(((FForall)originFormula).getPattern_id())){
            //同一个pattern
